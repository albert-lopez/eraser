...
if (arg[1] == "g"):
    qos = "-L 500 -E 20"
run_command(ssh_nodes_info["c"],"sudo killall iporinad", True)
run_command(ssh_nodes_info["s1"],"sudo killall iporinad", True)
run_command(ssh_nodes_info["c"],"sudo iporinad %s -c ./iporinad_c.conf" % (qos), True)
run_command(ssh_nodes_info["s1"],"sudo iporinad %s -c ./iporinad_s.conf" % (qos), True)
