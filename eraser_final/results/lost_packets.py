...
def processLine(fd,file_name):
    ...
    if (len(camps)!=8):
        ...
        sys.exit()
    lost1 = int(camps[3])+int(camps[4])
    lost2 = int(camps[6])+int(camps[7])
    max = int(camps[0])
    return ((lost1,lost2,max))
...
def processFile2(file_name):
    ...
    if (lost1 != 0 or lost2 != 0):
        print ("Drop qtamux: %s -> Chery 0: %d  , Chery 1: %d" %(file_name,lost1,lost2))    
...
traffic_types = ["vlc_video","py","video_call","voip","vpn","hd_video","interactive"]
qos_st_dic = {}
